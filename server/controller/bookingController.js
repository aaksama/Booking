...
exports.sendBookingOTP = async (req, res) => {
    ...
    await OTP.findOneAndDelete({emal: req.user.email, action: 'event_booking'});
    await OTP.create({emal: req.user.email, otp: otp, action: 'event_booking'});
...
exports.bookEvent = async (req, res) => {
    ...
    const existingBooking = await Booking.findOne({userId: res.user._id, eventId})
    if(existingBooking) {
...
exports.confirmBooking = async (req, res) => {
    ...
    const booking = await Booking.findById(req.params.id).populate('evnetId');
    if(!booking) {
